...
export function validateBlueDartConfig(): BlueDartConfigValidation {
  ...
  // APIGEE token auth credentials (for dynamic JWT token generation)
  const clientId = process.env.BLUEDART_CLIENT_ID?.trim()
  const clientSecret = process.env.BLUEDART_CLIENT_SECRET?.trim()

  if (!clientId || !clientSecret) {
    warnings.push(
      'BLUEDART_CLIENT_ID and BLUEDART_CLIENT_SECRET not set. These are required for APIGEE JWT token generation. Get them from https://developer.dhl.com/ App credentials. Falling back to BLUEDART_LICENSE_KEY as static token.'
    )
  }
